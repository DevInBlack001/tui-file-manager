...
- Images were requested from `chafa` sized for the *outer* preview pane rect (border included) but blitted starting just inside the border, so every image was rendered 2 columns and 2 rows larger than the space it was actually drawn into and overflowed past the pane's own edges - and since that overflow fell outside the area later cleared on navigation, the overflowing strip stuck around even after moving to a different file. Preview requests now use the pane's inner content dimensions (matching exactly where the image is blitted), so images render at the correct size and don't overflow in the first place.
